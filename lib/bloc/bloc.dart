...
class BlocIn with FirebaseAuthBloc, PickImageWithBloc {
  ...
  StreamController<bool> _codeSentStreamController =
      StreamController.broadcast(); //This stream controller is used for the code.
  StreamController<AllFeeds> _allFeedsStreamController =
      StreamController.broadcast(); //This stream controller is used for getting the feeds.
  StreamController<File> _pickImageStreamController =
      StreamController.broadcast(); //This stream controller is used to getting images.
  StreamController<bool> _uploadCompletedController = StreamController.broadcast();
  Repository _repository = Repository(); // This is the repo where all the fetching is found.
  ...
  uploadFeedToDB(File file, String caption) {
    _repository.uploadFeed(file, token, caption).then((value) {
      if (value != null) {
  ...
  likeFeed(String feedId) {
    _repository.likeFeed(token, feedId).then((value) {
      if (value != null) {
  ...
  Future fetchAllFeeds({String pageNumber}) async {
    ...
    AllFeeds allFeeds = await _repository.getAllFeeds(token, pageNumber);
    _logger.d(allFeeds.feedModel.length.toString() + " The length is "+ allFeeds.page.toString() + " PageNumber " + pageNumber);
  ...
  StreamController<bool> get codeSentStreamController => _codeSentStreamController;
  StreamController<bool> get uploadCompleteController => _uploadCompletedController;
}
